...
class CaptchaMiddleware:
    ...
    def handle_captcha(self, request, spider):
        ...
        try:
            ...
            raise Exception("未获取到滑动拼图验证码")

        try:
            image_bg, image_fullbg = self.capture_canvas_image()
            ...
            raise Exception("解析滑动拼图失败")

        try:
            WebDriverWait(self.driver, 10).until(lambda d: 'anjuke.com' in urlparse(d.current_url).netloc)
            return
        except Exception as e:
            raise Exception("最终未跳转至安居客详情页")
